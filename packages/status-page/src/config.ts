/**
 * The shape of status.config.ts - the one file an instance edits to make this
...
export type IconsConfig = {
  /** Paths under the frontend's public/. Omit any you do not ship. */
  favicon?: string;
  icon192?: string;
  icon512?: string;
  appleTouch?: string;
...
export type StatusConfig = {
   ...
   */
  indexable: boolean;
  /** 1200x630 share image path under the frontend's public/, or null for none. */
  ogImage?: string | null;
  /** Favicon and home-screen icons. Omit to ship none. */
  icons?: IconsConfig;
  /** Logo path under the frontend's public/ (e.g. "/logo.svg"), or null to hide it. */
  logo: string | null;
  /**
...
/** Identity helper: gives status.config.ts full type checking and editor hints. */
export function defineConfig<const T extends StatusConfig>(config: T): T {
  return config;
}
