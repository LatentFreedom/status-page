...
/**
 * Builds the worker's module handlers from the instance config. The instance's
 * src/index.ts is one line - `export default createStatusWorker(config)` - so
 * every probe and feed fix ships as a package bump, never as a copied file.
 */
export function createStatusWorker(config: { services: ServiceConfig[] }) {
  ...
  return {
    async fetch(request: Request, env: Env): Promise<Response> {
      ...
      if (pathname === "/api/v1/uptime") {
        return handleFeed(request, env, CORS_HEADERS, services);
      }
      ...
      if (pathname === "/api/v1/probe" && request.method === "POST") {
        ...
        return json(await runProbe(env, services));
      }
    ...
    async scheduled(
      ...
      ctx.waitUntil(runProbe(env, services));
    },
  };
}
